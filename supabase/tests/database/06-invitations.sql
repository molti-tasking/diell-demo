...
SELECT row_eq(
               $$ insert into public.invitations (organization_id, account_role, token, invitation_type) values ('d126ecef-35f6-4b5d-9f28-d9f00a9fb46f', 'owner', 'test_member_single_use_token', 'one_time') returning 1 $$,
               ROW (1),
               'Owners should be able to add one_time invitations for new members'
           );

SELECT row_eq(
               $$ insert into public.invitations (organization_id, account_role, token, invitation_type) values ('d126ecef-35f6-4b5d-9f28-d9f00a9fb46f', 'member', 'test_member_24_hour_token', '24_hour') returning 1 $$,
               ROW (1),
...
SELECT row_eq(
               $$ select json_object_keys(create_invitation('d126ecef-35f6-4b5d-9f28-d9f00a9fb46f'::uuid, 'member'::public.account_role, '24_hour'::public.invitation_type))$$,
               ROW ('token'::text),
               'Owners should be able to add 24_hour invitations for new members with create_invitation'
           );

-- invitations should work
SELECT row_eq(
               $$ select json_array_length(get_organization_invitations('d126ecef-35f6-4b5d-9f28-d9f00a9fb46f')) $$,
               ROW (3),
...
insert into public.invitations (organization_id, account_role, token, invitation_type, id)
...
SELECT ok(
               (select 'd126ecef-35f6-4b5d-9f28-d9f00a9fb46f' NOT IN
                       (select public.get_organizations_with_role('member'))),
...
SELECT throws_ok(
               $$ insert into public.invitations (organization_id, account_role, token, invitation_type) values ('d126ecef-35f6-4b5d-9f28-d9f00a9fb46f', 'member', 'test_permissions_token', 'one_time') returning 1 $$,
...
SELECT throws_ok(
               $$ insert into public.invitations (organization_id, account_role, token, invitation_type) values ('d126ecef-35f6-4b5d-9f28-d9f00a9fb46f', 'owner', 'test', 'one_time') returning 1 $$,
               'new row violates row-level security policy for table "invitations"'
           );
...
insert into public.invitations (organization_id, account_role, token, invitation_type)
